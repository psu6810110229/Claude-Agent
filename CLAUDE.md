...
- **Config (off by default):** `LINE_ENABLED` (DB `line_enabled` overrides), `LINE_EXPORT_DIR` (default gitignored `data/line-exports/`), `LINE_MAX_RESULTS` (50), `LINE_CHAT_CONTEXT_CAP` (8). **Required local config to surface chats:** set BOTH `LINE_ENABLED=1` and `LINE_EXPORT_DIR=<absolute path to LINEExports>` in the backend `.env` (repo-root `.env` or `packages/backend/.env` — the only two paths the loader reads; gitignored, never commit). Setting only `LINE_EXPORT_DIR` leaves the connector disabled; restart backend after editing. See `automation/README.md` → "Backend connector — required local config".
